...
public class Main {
	...
	public static void main(String[] args) throws IOException{
		...
		listaGrupos.stream()
				.map(grupo -> "Vencedor(a): ".concat(iniciarGincana(grupo)))
				.forEach(System.out::println);
									
		br.close();
	}
	
	static List<Aluno> gerarGrupo(int N) throws IOException {
		List<Aluno> grupo = new ArrayList<Aluno>();		
		int i=0;
		while(i<N) {
			StringTokenizer linha = ler(" ");				
	...
	static StringTokenizer ler(String divisor) throws IOException {
		return new StringTokenizer(br.readLine(), divisor);
	}
